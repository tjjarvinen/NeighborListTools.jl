...
function CellList(data::AtomsBase.FastSystem, cutoff, cell_size_suggestion::AbstractVector{T}; boundaryconditions=:periodic, sortstyle = :unsorted) where T
    ...
    celllist = Cell{T}.(collect(Iterators.product(range.(1,cellNs)...)),sortstyle) # generate array of empty & unsorted Cells
...
"""
function cellsort!(cell, method::Symbol)
    cell.sorttype = method
    cellsort!(cell)
    cell
end
function cellsort!(cell)
