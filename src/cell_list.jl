mutable struct Cell{TA, TI, TT}
    ...
    originalindex::TI
    "how are the elements of the cell sorted? Currently implemented: :alphaelement, :atomicnumber and :unsorted"
    sorttype::Symbol
    "vector of element types, stored as e.g. [Ti, Al, ...]"
    elementtypes::Vector{Symbol}
...
mutable struct CellList{T, TS, TB, TC, TL, ND} # ND is number of dimensions
    ...
    "Which cell are the atoms in the original data now in?"
    originalindstocell::TL
    # TODO: What other fields are needed?
end
...
# Create a CellList out of an AtomsBase.FastSystem input
...
function CellList(data::AtomsBase.FastSystem, cutoff, cell_size_suggestion::AbstractVector{T}; boundaryconditions=:periodic, sortstyle = :unsorted) where T
    ...
    cellNs = Int.(floor.(norm.(box) ./ cell_size_suggestion))                 # number of cells in each direction
    cellshape = box ./ cellNs                                           # compute cell axes & size since suggestion
                                                                        ## may not match integer multiple of box
    @inbounds celllist = Cell{float(T)}.(collect((Iterators.product(range.(1,cellNs)...))), sortstyle) # generate array of empty & unsorted Cells
    @inbounds coords = _getnormedcellids.((data.position,),(cellshape,),(cellNs,),1:length(data.position))    # Find out which cell ID each atom belongs in
    if boundaryconditions == :periodic
        # Account for periodicity in positions
        @inbounds projectedpos = _getperiodicprojectedpos.(ustrip.(data.position),(ustrip.(box),)).*unit(eltype(box[1]))
    else
    ...
    return CellList{typeof(celllist),typeof(cellshape),typeof(box),typeof(cutoff),typeof(coords),size(box,1)}(celllist, cellshape, box, cutoff, boundaryconditions, coords)
end
...
"""
# TODO: change loop to be multi-threaded by rewriting it to loop over cellids?
function massivelyaddparticles!(celllist, positions, elementtypes, cellcoords)
    # TODO: We use push! for now for a quick implementation but a first guess pre-allocation + extend when needed approach is a better idea
    @inbounds for j = 1:length(positions)
        push!(celllist[cellcoords[j]...].position, positions[j]) # append atom
        push!(celllist[cellcoords[j]...].elementtypes, elementtypes[j]) # append element of atom
...
function cellsort!(cell::Cell)
    # TODO: Think about contiguity in memory, possibly need a deepcopy somewhere?
    ...
    elseif cell.sorttype == :atomicnumber
        perm = sortperm(getfield.(element.(cell.elementtypes),:number))
        cell.position = cell.position[perm]
        cell.elementtypes = cell.elementtypes[perm]
        cell.originalindex = cell.originalindex[perm]
